...
| Frozen once `ACTIVE` | Editable in place, recorded in `campaign_amendment` |
| --- | --- |
...
| `run_id` | ULID | PK |
| `campaign_id` | ULID | FK |
| `fencing_token` | bigint | Monotonic per campaign; checked on every state change this run makes |
...
| `proposed_at_experiment_count` | int | Completed-experiment count at proposal time — drives staleness |
...
Note that this is a **column, not a count over history**. Anything the control path needs is a
column; deriving it by counting log rows is the move that turns the debug log into a second source
of truth (doc 02 §3, rule 2).
...
| `invalidated_at` / `invalidation_reason` | timestamptz? / text? | Retraction. Never deletes metrics — doc 07's confirmation policy needs retracted results visible |
...
| `state` | enum | See `03-lifecycle.md` |
| `infra_attempt_count` | int | Counts toward `max_infra_reclassify` (doc 06). A column, not a log query |
...
SQL view over the state tables — cheap to compute at D17 scale, and never stale by construction:
